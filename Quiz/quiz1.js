...
loading();
/**
 * It takes the current question from the objectofquiz array and displays it on the page.
 */
function loading() {
  deselectionAns();
  const current = objectofquiz[currentQuestion];
...
/* An event listener that listens for a click on the submit button. When the submit button is clicked,
  ...
  currentQuestion variable. If the currentQuestion variable is less than the length of the objectofquiz
  array, it calls the loading function. If the currentQuestion variable is not less than the length of
  the objectofquiz array, it replaces the contents of the box element with a message that displays the
  score and a button that reloads the page. */

  let limit = 5;
  const time=document.querySelector("h1")
  time.innerHTML=`00:${limit}`;
  ...
  function clickevent(){
    submitbtn.addEventListener("click", () => {
        ...
        if (myAns) {
          if (myAns == objectofquiz[currentQuestion].correct) {
            score++;
          }
          currentQuestion++;
      
          if (currentQuestion < objectofquiz.length) {
            loading();
          } else {
            box.innerHTML = ` <h2 style="color:Blcak ; background: red ;  font-size:  25px; border: 4px solid green;" > your score is ${score}</h2>
